...
if [ -f "$SAVED_CONF" ]; then
  ...
  while IFS='=' read -r k v; do
    case "$k" in
      PROJECT_NAME|WORKSPACE_MOUNT|CONTAINER_SCOPE|BASE_IMAGE|EXTRA_PACKAGES|TIMEZONE|SELECTED_SERVICES|POSTGRES_DBS|DOCKER_MODE)
        v="${v%\"}"; v="${v#\"}"
...
# 3. Workspace mount mode: Parent-Verzeichnis ist der Default (ganzes Workspace
# sichtbar, alle Nachbar-Repos eingeschlossen). Ein gespeicherter Wert gewinnt
# beim Reconfigure.
...
# Container-Scope: nur bei parent-Mount wählbar. Bei project-Mount ist jeder
# Worktree an seinen eigenen Container gebunden.
if [ "$WORKSPACE_MOUNT" = "parent" ]; then
  prompt_header "Container-Scope"
  case "${SAVED_CONTAINER_SCOPE:-shared}" in
    per-worktree) PROMPT_DEFAULT=2 ;;
    *)            PROMPT_DEFAULT=1 ;;
  esac
  prompt_select "Container-Scope über Worktrees/Branches?" \
    "Shared        (ein Container für alle Worktrees/Branches)" \
    "Per-Worktree  (eigener Container je Worktree-Verzeichnis)"
  case "$REPLY" in
    Shared*) CONTAINER_SCOPE="shared" ;;
...
GIT_VOLUME="      # .git ist Teil des Workspace-Mounts"
