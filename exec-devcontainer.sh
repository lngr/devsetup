...
if [ "$SYNC_CLAUDE_CONFIG" = true ] && [ -d "$HOME/.claude" ]; then
  ...
  if [ "$USE_DOCKER_EXEC" = true ]; then
    ...
    devcontainer exec --workspace-folder . bash -c 'sudo mkdir -p /home/vscode/.claude && sudo chown vscode:vscode /home/vscode/.claude' || true
  fi
...
" 2>/dev/null) && {
      if [ "$USE_DOCKER_EXEC" = true ]; then
        ...
        echo "$CLAUDE_JSON" | devcontainer exec --workspace-folder . bash -c 'cat > /home/vscode/.claude.json'
      fi
    } || true
  fi
fi
